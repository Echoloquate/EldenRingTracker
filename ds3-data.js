...
const TYPE_TO_CAT = {
  ...
  'Ashes': 'key',
  'Major boss': 'major',
  'Powerful enemy': 'enemy',
  'Small crystal lizard': 'enemy',
  'Flask upgrade': 'flask',
  'Flask upgrade + Powerful enemy': 'flask',
  'Flask upgrade + Chest': 'flask',
  'Shop': 'shop',
  'Ring': 'ring',
  'Chest + Ring': 'ring',
  'Chest': 'chest',
...
function parseData(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    const typePrefix = inIdx !== -1 ? trimmed.slice(0, inIdx) : null;
    const cat = TYPE_TO_CAT[typePrefix] || 'chest';
    ...
    if (replacesIdx !== -1) {
      ...
      currentLocation.items.push({ desc, replaces, cat });
    } else {
      currentLocation.items.push({ desc: afterColon, replaces: null, cat });
    }
  }
